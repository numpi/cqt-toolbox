function [x1, U, y, info] = eig(A, x0, varargin)
% function [x1, flg, res, vres, vresest, it, U, Y, y] = eig(A, x0, algo, maxit, epsil, verbose, residual, advpx, digits)
% Compute an eigenvalue of A = cqt(am,ap,E) by means of
% fixed point iteration starting by an initial guess x0
% Input:
%   am, ap, E so that A = cqt(am,ap,E)
%   x0 : initial guess
%   maxit: maximum number of iterations
%   algo: fixed point algorithm
...
% verbose: if true then some information is printed at run time
% epsil: error bound
% residual: if true, compute the residual error
% advpx: true if the multiprecision toolbox Advanpix is used
% digits: number of decimal digits used in the computation
% Output:
% x1 : eigenvalue
% flg :  0 p=0 No values in the connected component containing x1 are eigenvalues
%              (p is the number of zeros of a(z)-x1 of modulus <1)
...
%        7 exceeded max number of iterations
% res :  residual error in the first q components of the equation Av-lambda v=0
% vres : residual errors per step 
% vresest: residual errors per step estimated by sv(end)/sv(1) for sv = svd(Y) 
% it: number of iterations
% U either the pxp Vandermonde or G=F^p, where F is the Frobenius matrix
% Y = W*[V;VD;VD^2;...] or Y = W*[I;G;G^2;...]
% y: vector such that [VDy; VD^2y; VD^3y; ...] or [Gy; G^2y; G^3y;...], is the corresponding eigenvector
% it: number of iterations
...
%-2  select the algorithm 
     ...
     x0 = x1;
     if verbose
        fprintf('      rel_correction = %e, flag = %d\n', single(err), info.flg);
     end
...
%    Compute the vector y by means of SVD, and the residual error
     ...
     if verbose
        fprintf('      residual = %e, resalt = %e\n', single(info.res), single(resalt));
     end
